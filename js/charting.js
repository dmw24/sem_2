...
function getUeTypeColor(typeName, index = 0) { return ueTypeColorMapping[typeName] || getTechColor(typeName, index); } // Fallback to tech color logic
    ...
    if (balanceChartsSection && !balanceChartsSection.classList.contains('hidden')) {
        ...
        if (balanceSector === 'Power') {
            ...
            pedDatasets = primaryFuels.map((fuel, fuelIndex) => ({ label: fuel, data: chartLabels.map(y => getValue(yearlyResults, [y, 'powerPrimaryInputsByFuel', fuel], 0) / GJ_PER_EJ), backgroundColor: getTechColor(fuel, fuelIndex), })).filter(ds => ds.data.some(v => v > 1e-9));
             ...
             const h2InputFuels = Object.keys(getValue(yearlyResults[chartLabels[0]], ['hydrogenPrimaryInputsByFuel'], {})); // Get potential input fuels from first year
        ...
        } else { if (chartInstances['fecFuelChart']) { chartInstances['fecFuelChart'].destroy(); delete chartInstances['fecFuelChart']; } } // Destroy if hidden
        ...
        } else { if (chartInstances['ueFuelChart']) { chartInstances['ueFuelChart'].destroy(); delete chartInstances['ueFuelChart']; } } // Destroy if hidden
