...
function runModelCalculation(structuredData, userInputParameters) {
    ...
    for (const year of years) {
        ...
        const currentHydrogenProdMix_yr = yearlyResults[year].hydrogenProdMix; const fecHydrogen = currentFecByFuel['Hydrogen'] || 0; let hydrogenPrimaryInputsByFuel = primaryFuels.concat(endUseFuels).reduce((acc, f) => ({ ...acc, [f]: 0 }), {}); hydrogenTechs.forEach(ht => { const mixPercent = getValue(currentHydrogenProdMix_yr, [ht], 0); const mixFraction = mixPercent / 100; const unitConsMap = getValue(hydrogenTechUnitEnergyCons, [ht], {}); Object.keys(unitConsMap).forEach(f_input => { const unitCons = unitConsMap[f_input]; const demand = fecHydrogen * mixFraction * unitCons; hydrogenPrimaryInputsByFuel[f_input] = (hydrogenPrimaryInputsByFuel[f_input] || 0) + demand; }); }); yearlyResults[year].hydrogenPrimaryInputsByFuel = hydrogenPrimaryInputsByFuel; const currentEcPostHydrogen = { ...currentFecByFuel }; delete currentEcPostHydrogen['Hydrogen']; Object.keys(hydrogenPrimaryInputsByFuel).forEach(f => { if (hydrogenPrimaryInputsByFuel[f] > 0.001) { currentEcPostHydrogen[f] = (currentEcPostHydrogen[f] || 0) + hydrogenPrimaryInputsByFuel[f]; } }); yearlyResults[year].ecPostHydrogen = currentEcPostHydrogen;

        // 6. Power Transformation
        const currentPowerProdMix_yr = yearlyResults[year].powerProdMix; const ecElectricity = currentEcPostHydrogen['Electricity'] || 0; let powerPrimaryInputsByFuel = primaryFuels.concat(endUseFuels).reduce((acc, f) => ({ ...acc, [f]: 0 }), {}); powerTechs.forEach(pt => { const mixPercent = getValue(currentPowerProdMix_yr, [pt], 0); const mixFraction = mixPercent / 100; const unitConsMap = getValue(powerTechUnitEnergyCons, [pt], {}); Object.keys(unitConsMap).forEach(f_input => { const unitCons = unitConsMap[f_input]; const demand = ecElectricity * mixFraction * unitCons; powerPrimaryInputsByFuel[f_input] = (powerPrimaryInputsByFuel[f_input] || 0) + demand; }); }); yearlyResults[year].powerPrimaryInputsByFuel = powerPrimaryInputsByFuel; const currentEcPostPower = { ...currentEcPostHydrogen }; delete currentEcPostPower['Electricity']; Object.keys(powerPrimaryInputsByFuel).forEach(f => { if (powerPrimaryInputsByFuel[f] > 0.001) { currentEcPostPower[f] = (currentEcPostPower[f] || 0) + powerPrimaryInputsByFuel[f]; } }); yearlyResults[year].ecPostPower = currentEcPostPower;
