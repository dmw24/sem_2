// js/uiController.js
// Version: Added Chart View Control
...
const createSCurveParamInputs = (paramKey, baseValue) => { /* ... (unchanged) ... */
    const div = document.createElement('div'); div.className = 's-curve-inputs'; const sanitizedParamKey = sanitizeForId(paramKey); div.id = `sCurveInputs_${sanitizedParamKey}`; const targetInputId = `sCurveTarget_${sanitizedParamKey}`; const targetYearInputId = `sCurveTargetYear_${sanitizedParamKey}`; const steepnessInputId = `sCurveSteepness_${sanitizedParamKey}`; const targetLabel = document.createElement('label'); targetLabel.htmlFor = targetInputId; targetLabel.textContent = `Target Share (%):`; const targetInput = document.createElement('input'); targetInput.type = 'number'; targetInput.id = targetInputId; targetInput.min = '0'; targetInput.max = '100'; targetInput.step = '1'; targetInput.value = Math.min(100, Math.max(0, baseValue + 5)).toFixed(1); div.appendChild(targetLabel); div.appendChild(targetInput); const targetYearLabel = document.createElement('label'); targetYearLabel.htmlFor = targetYearInputId; targetYearLabel.textContent = `Target Year:`; const targetYearInput = document.createElement('input'); targetYearInput.type = 'number'; targetYearInput.id = targetYearInputId; targetYearInput.min = String(uiStartYear + 1); targetYearInput.max = String(uiEndYear + 10); targetYearInput.step = '1'; targetYearInput.value = String(uiEndYear); div.appendChild(targetYearLabel); div.appendChild(targetYearInput); const steepnessLabel = document.createElement('label'); steepnessLabel.htmlFor = steepnessInputId; steepnessLabel.textContent = `Steepness (1-10):`; const steepnessInput = document.createElement('input'); steepnessInput.type = 'number'; steepnessInput.id = steepnessInputId; steepnessInput.min = '1'; steepnessInput.max = '10'; steepnessInput.step = '1'; steepnessInput.value = '5'; div.appendChild(steepnessLabel); div.appendChild(steepnessInput); return div; };
...
// --- Input Gathering ---
function getUserInputsAndParams(structuredData) { /* ... (unchanged) ... */
    const { sectors, subsectors, technologies, powerTechs, hydrogenTechs, allEndUseSubsectors, endYear } = structuredData; const userInputParameters = { activityGrowthFactors: {}, techBehaviorsAndParams: {} }; (allEndUseSubsectors || []).forEach(({ sector, subsector }) => { const inputId = `growth_${sanitizeForId(sector)}_${sanitizeForId(subsector)}`; const inputElement = document.getElementById(inputId); const growthPercent = inputElement ? parseFloat(inputElement.value) : 0; const growthFactor = isNaN(growthPercent) ? 1.0 : 1 + (growthPercent / 100); userInputParameters.activityGrowthFactors[`${sector}|${subsector}`] = growthFactor; }); const readTechInputs = (categoryType, categoryKey, techList) => { (techList || []).forEach(t => { const paramKey = `${categoryType}|${categoryKey}|${t}`; const sanitizedParamKey = sanitizeForId(paramKey); const behaviorEl = document.getElementById(`behavior_${sanitizedParamKey}`); const behavior = behaviorEl ? behaviorEl.value : 'fixed'; const techParams = { behavior: behavior }; if (behavior === 's-curve') { const targetEl = document.getElementById(`sCurveTarget_${sanitizedParamKey}`); const targetYearEl = document.getElementById(`sCurveTargetYear_${sanitizedParamKey}`); const steepnessEl = document.getElementById(`sCurveSteepness_${sanitizedParamKey}`); const defaultTargetYear = structuredData.endYear || uiEndYear; techParams.targetShare = targetEl ? parseFloat(targetEl.value) : 0; techParams.targetYear = targetYearEl ? parseInt(targetYearEl.value) : defaultTargetYear; techParams.steepness = steepnessEl ? parseInt(steepnessEl.value) : 5; if (isNaN(techParams.targetShare)) techParams.targetShare = 0; if (isNaN(techParams.targetYear)) techParams.targetYear = defaultTargetYear; if (isNaN(techParams.steepness) || techParams.steepness < 1 || techParams.steepness > 10) techParams.steepness = 5; } userInputParameters.techBehaviorsAndParams[paramKey] = techParams; }); }; sectors.forEach(s => { if (subsectors[s]){ subsectors[s].forEach(b => { readTechInputs('Demand', `${s}|${b}`, technologies[s]?.[b] || []); }); } }); readTechInputs('Power', 'Power', powerTechs); readTechInputs('Hydrogen', 'Hydrogen', hydrogenTechs); return userInputParameters; }
...
function handleChartViewChange() {
    ...
    // Optional: Could trigger a chart redraw here if needed, but often not necessary
...
/**
 * Sets up event listeners for the Run button and dropdowns.
...
function setupEventListeners(appState) { // Accepts appState
    ...
    const chartViewSelect = document.getElementById('selectChartView'); // Get the new dropdown
    ...
    if (!runButton) { console.error("Run Model button not found!"); return; }
    if (!subsectorSelect) { console.error("Subsector select dropdown not found!"); return; }
    if (!chartViewSelect) { console.error("Chart view select dropdown not found!"); return; } // Check new dropdown

    // Run Model Button Click Handler
    runButton.onclick = async () => {
        runButton.disabled = true; runButton.textContent = 'Calculating...';
        console.log("Run button clicked. Gathering inputs and running model...");
        try {
            const userInputs = getUserInputsAndParams(structuredData);
            if (typeof runModelCalculation !== 'function') { throw new Error("runModelCalculation function is not defined."); }
            const modelResults = await runModelCalculation(structuredData, userInputs);
            appState.latestResults = modelResults; // Update shared state
            if (typeof updateCharts !== 'function') { throw new Error("updateCharts function is not defined."); }
            updateCharts(appState.latestResults, structuredData); // Pass updated state
        } catch (error) { console.error("Error during model execution or chart update:", error); alert(`An error occurred: ${error.message}.`); }
        finally { runButton.disabled = false; runButton.textContent = 'Run Model & Update Charts'; }
    };

     // Subsector Dropdown Change Handler (for subsector details view)
     subsectorSelect.onchange = () => {
         ...
         if (appState.latestResults) { // Read from appState
              updateCharts(appState.latestResults, structuredData); // Pass latest results and config
         } else { console.warn("No model results available for subsector change."); }
     };

     // Chart View Dropdown Change Handler
     chartViewSelect.onchange = handleChartViewChange; // Assign the handler function

     // Initial setup for visibility based on default selection
     handleChartViewChange(); // Call once to set initial visibility state
