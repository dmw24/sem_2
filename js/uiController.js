// js/uiController.js
// Version: Added Midpoint Year Input for S-Curve
...
/**
 * Creates the HTML elements for S-curve parameters (Target Share, Target Year, Midpoint Year).
...
const createSCurveParamInputs = (paramKey, baseValue) => {
    ...
    // *** REMOVED Steepness Input ***
    // const steepnessLabel = document.createElement('label');
    // steepnessLabel.htmlFor = steepnessInputId; steepnessLabel.textContent = `Steepness (1-10):`;
    // const steepnessInput = document.createElement('input');
    // steepnessInput.type = 'number'; steepnessInput.id = steepnessInputId; steepnessInput.min = '1'; steepnessInput.max = '10'; steepnessInput.step = '1'; steepnessInput.value = '5';
    // div.appendChild(steepnessLabel); div.appendChild(steepnessInput);

    // *** ADDED Midpoint Year Input ***
    const midpointYearLabel = document.createElement('label');
    midpointYearLabel.htmlFor = midpointYearInputId;
    midpointYearLabel.textContent = `Midpoint Year (Fastest Growth):`;
    const midpointYearInput = document.createElement('input');
    midpointYearInput.type = 'number';
    midpointYearInput.id = midpointYearInputId;
    midpointYearInput.min = String(uiStartYear + 1); // Midpoint should be after start
    midpointYearInput.max = String(uiEndYear + 5); // Allow midpoint slightly beyond target year
    midpointYearInput.step = '1';
    // Default midpoint roughly halfway between start and default target year
    const defaultMidpoint = Math.round(uiStartYear + (parseInt(targetYearInput.value, 10) - uiStartYear) / 2);
    midpointYearInput.value = String(defaultMidpoint);
    div.appendChild(midpointYearLabel);
...
// --- UI Initialization ---
function initializeSidebarInputs(structuredData) { /* ... (logic unchanged, uses updated createTechInput) ... */
    ...
    const readTechInputs = (categoryType, categoryKey, techList) => {
        (techList || []).forEach(t => {
            ...
            if (behavior === 's-curve') {
                ...
                // Read midpoint year instead of steepness
                techParams.midpointYear = midpointYearEl ? parseInt(midpointYearEl.value, 10) : Math.round(startYear + (endYear - startYear) / 2); // Default halfway
                ...
                if (isNaN(techParams.midpointYear) || techParams.midpointYear < startYear) {
                     // Default midpoint halfway between start and target year if invalid
                    techParams.midpointYear = Math.round(startYear + (techParams.targetYear - startYear) / 2);
                }
            }
 ...
 */
function setupEventListeners(appState) { /* ... (event handler logic unchanged, uses updated getUserInputsAndParams) ... */
